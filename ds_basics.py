...
def sort_df(df):
    keywords = ["gifted","talented","mensa","high IQ"]
    final = df[~(df['score'] <= 0.31)]
    purged = df[~(df['score'] >= 0.31)]
    rescued = purged[df['title'].astype(str).str.contains('|'.join(keywords),case=False) | df['abstract'].astype(str).str.contains('|'.join(keywords),case=False)]
