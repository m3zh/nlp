...
def frontiersin_df_feeder(keywords):
    ...
    for i in range(5): # <-- this is the number times you want the page to load; increase as you like (1 load = 25 results)
        elem = "#article-results > ul > li:last-child"
        search= "document.querySelector(\'" + elem + "\').scrollIntoView();"
    ...
    js_script = '''\
    var s = []
    document.querySelectorAll('script').forEach(script => s.push(script.src));
    return s;
    '''
    scripts = browser.execute_script(js_script)
    print(scripts[:3])
    url_list = []
    for s in scripts:
        if 'altmetric' in s:
